...
class MaterialDetection:
    ...
    def validate_detection(self, utility, message, rule_set=None):
        ...
        #
        #     # If the material state has changed during the 10-minute cycle, start the 20-second countdown
        #         if self.key_string_current_material_state != self.key_string_prev_material_state and not self.key_string_start_20sec_cycle:
        #             self.key_string_start_20sec_cycle = True  # Initiate the 20-second confirmation cycle
        #             self.key_string_conta_time_stamp = time.strftime("%Y-%m-%d %H:%M:%S")
        #             utility.redis_insert_data("contamination_image", message['frame']['image'])  # Save contamination image
        #             return message
        #
        #         # If the state remains consistent for 20 seconds, confirm contamination and log the event
        #         if self.key_string_start_20sec_cycle and not self.key_string_contam:
        #             self.key_string_contam = True  # Set the contamination confirmation flag
        #             state_img = utility.redis_get_data("state_image")  # Retrieve the previous state image
        #             conta_img = utility.redis_get_data("contamination_image")  # Retrieve contamination image
        #             curr_img = message['frame']['image']  # Current image from the message packet
        #
        #             # Create a stacked image showing previous state, contamination, and current state
        #             stacked_image = utility.get_stacked_image(state_img, conta_img, curr_img)
        #             message['frame']['image'] = utility.encode_image(stacked_image)  # Encode and update the message image
        #
        #             # Add detailed contamination event information to the message
        #             message['event'] = {
        #                 "timestamp": self.key_string_conta_time_stamp,
        #                 "type": "Contamination Detection",
        #                 "severity": "High",
        #                 "status": "Confirmed"
        #             }
        #
        #             # Reset state flags for the next detection cycle
        #             self.key_string_start = False
        #             self.key_string_start_10min_cycle = False
        #             self.key_string_start_20sec_cycle = False
        #             self.key_string_contam = False
        #             return message
        #
        #     # If the 10-minute cycle completes without contamination, reset state flags
        #     if self.key_string_start_10min_cycle and not self.key_string_start_20sec_cycle:
        #         self.key_string_start = False
        #         self.key_string_start_10min_cycle = False
        #         self.key_string_prev_material_state = None
        #         self.key_string_current_material_state = None
        #         self.key_string_prev_time_stamp = None
        #         self.key_string_conta_time_stamp = None
        #         return message
        #
        # except Exception as e:
        #     # Print the error and traceback in case of an exception for debugging
        #     print(f"An error occurred: {e}")
        #     traceback.print_exc()
        # return message  # Return the updated message regardless of the exception
